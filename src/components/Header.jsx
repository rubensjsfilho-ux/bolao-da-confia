...
const NAV = [
  ...
  { to: '/campeao',   label: 'Final',   Icon: Gift },
]

// ── Componente reutilizável de avatar (foto ou emoji) ─────────────────────────
export function Avatar({ photoUrl, emoji = '⚽', size = 36, border = true }) {
  return (
    <div style={{
      width: size, height: size, borderRadius: '50%',
      overflow: 'hidden', flexShrink: 0,
      border: border ? '2px solid #E2EAF0' : 'none',
      background: photoUrl ? 'transparent' : 'linear-gradient(135deg,#e8f5ee,#F4F6F9)',
...
export default function Header({ participant, onLogout }) {
  ...
  return (
    <>
        ...
        {participant && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ textAlign: 'right', display: 'none' }}>
              <div style={{ color: '#002855', fontWeight: 700, fontSize: 13 }}>{participant.name}</div>
            </div>
      ...
      {participant && (
        <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 50, background: '#fff', borderTop: '1px solid #E2EAF0', padding: '6px 8px 10px', display: 'flex', justifyContent: 'space-around', boxShadow: '0 -2px 12px rgba(0,40,85,0.07)' }}>
          {NAV.map(({ to, label, Icon }) => {
            const active = pathname === to
            return (
              <Link key={to} to={to} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, padding: '6px 14px', borderRadius: 12, textDecoration: 'none', transition: 'all .15s', background: active ? 'rgba(0,150,57,0.08)' : 'transparent' }}>
