...
function KnockoutMatchCard({ match, dbMatch, prediction, onSave, participantId }) {
  ...
  let borderColor, statusText, statusColor, statusBg
  if (isLocked)       { borderColor='rgba(255,255,255,0.08)'; statusText='🔒 Encerrado';  statusColor='rgba(255,255,255,0.3)'; statusBg='rgba(255,255,255,0.05)' }
  else if (!hasTeams) { borderColor='rgba(255,255,255,0.06)'; statusText='⏳ Indefinido'; statusColor='rgba(255,255,255,0.3)'; statusBg='rgba(255,255,255,0.03)' }
  else if (hasPred)   { borderColor='rgba(0,196,79,0.3)';    statusText='✓ Palpitado';  statusColor='#00c44f';               statusBg='rgba(0,196,79,0.08)'    }
  else                { borderColor='rgba(245,166,35,0.4)';  statusText='⚡ Palpitar';   statusColor='#F5A623';               statusBg='rgba(245,166,35,0.06)'  }

  return (
    <div style={{ background:'#0d1b2e', borderRadius:14, border:`1.5px solid ${borderColor}`, marginBottom:8, overflow:'hidden', boxShadow:'0 2px 12px rgba(0,0,0,0.3)' }}>
      <div onClick={()=>hasTeams&&!isLocked&&setOpen(o=>!o)}
        style={{ padding:'12px 14px', display:'flex', alignItems:'center', gap:10, cursor:hasTeams&&!isLocked?'pointer':'default', background:statusBg }}>
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            <span style={{ fontSize:20 }}>{team1 ? getFlag(team1) : '🏳️'}</span>
            <span style={{ fontSize:12, fontWeight:700, color:team1?'#fff':'rgba(255,255,255,0.3)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
              {team1 || (match.info?.split(' × ')[0]||'A definir')}
            </span>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:4 }}>
            <span style={{ fontSize:20 }}>{team2 ? getFlag(team2) : '🏳️'}</span>
            <span style={{ fontSize:12, fontWeight:700, color:team2?'#fff':'rgba(255,255,255,0.3)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
              {team2 || (match.info?.split(' × ')[1]||'A definir')}
            </span>
          </div>
          {match.date && <div style={{ color:'rgba(255,255,255,0.3)', fontSize:9, marginTop:5 }}>📍 {match.city} · {formatDate(match.date)}</div>}
        </div>

        {hasPred && !open && (
          <div style={{ background:'rgba(0,196,79,0.15)', border:'1px solid rgba(0,196,79,0.3)', borderRadius:10, padding:'6px 12px', textAlign:'center', flexShrink:0 }}>
            <div style={{ color:'#fff', fontWeight:900, fontSize:18 }}>{prediction.score1} × {prediction.score2}</div>
          </div>
        )}

        <div style={{ display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
          <span style={{ color:statusColor, fontWeight:800, fontSize:11 }}>{statusText}</span>
          {hasTeams&&!isLocked&&(open?<ChevronUp size={14} color="rgba(255,255,255,0.3)"/>:<ChevronDown size={14} color="rgba(255,255,255,0.3)"/>)}
        </div>
      </div>

      {open && hasTeams && !isLocked && (
        <div style={{ borderTop:'1px solid rgba(255,255,255,0.08)', padding:'14px', background:'rgba(0,0,0,0.2)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
            <div style={{ flex:1, textAlign:'center' }}>
              <div style={{ fontSize:28, marginBottom:4 }}>{getFlag(team1)}</div>
              <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.7)', marginBottom:8 }}>{team1}</div>
              <input type="number" min="0" max="20" value={s1} onChange={e=>setS1(e.target.value)}
                style={{ width:60, height:48, background:'rgba(255,255,255,0.08)', border:'1.5px solid rgba(255,255,255,0.15)', borderRadius:12, fontSize:24, fontWeight:900, textAlign:'center', color:'#fff', outline:'none', fontFamily:'Nunito,sans-serif' }}/>
            </div>
            <div style={{ color:'rgba(255,255,255,0.3)', fontWeight:900, fontSize:20 }}>×</div>
            <div style={{ flex:1, textAlign:'center' }}>
              <div style={{ fontSize:28, marginBottom:4 }}>{getFlag(team2)}</div>
              <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.7)', marginBottom:8 }}>{team2}</div>
              <input type="number" min="0" max="20" value={s2} onChange={e=>setS2(e.target.value)}
                style={{ width:60, height:48, background:'rgba(255,255,255,0.08)', border:'1.5px solid rgba(255,255,255,0.15)', borderRadius:12, fontSize:24, fontWeight:900, textAlign:'center', color:'#fff', outline:'none', fontFamily:'Nunito,sans-serif' }}/>
            </div>
          </div>
          <button onClick={handleSave} disabled={s1===''||s2===''||saving}
            style={{ width:'100%', background:saved?'#009639':'#F5A623', color:saved?'#fff':'#000', border:'none', borderRadius:10, padding:'12px', fontWeight:900, fontSize:14, cursor:'pointer', fontFamily:'Nunito,sans-serif', opacity:s1===''||s2===''?0.4:1 }}>
...
export default function KnockoutPredictions({ participant, onLogout }) {
  ...
  const totalPreds = ROUNDS.reduce((acc,r) =>
    acc + Array.from({length:r.matchCount},(_,i)=>`${r.id}_${i+1}`).filter(id=>predictions[id]&&dbMatches[id]?.team1).length, 0)
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#060d18', paddingBottom:80 }}>
      ...
      {/* Hero */}
      <div style={{ background:'linear-gradient(135deg,#1a0040 0%,#2d0060 50%,#002855 100%)', padding:'70px 16px 24px', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', top:-40, right:-40, width:180, height:180, borderRadius:'50%', background:'rgba(123,47,190,0.15)', pointerEvents:'none' }}/>
        ...
        <h1 style={{ color:'#fff', fontWeight:900, fontSize:28, margin:'0 0 4px', letterSpacing:-1, textTransform:'uppercase' }}>PALPITES · MATA-MATA</h1>
        <p style={{ color:'rgba(255,255,255,0.4)', fontSize:12, margin:'0 0 16px' }}>Palpite em cada confronto</p>
        {totalAvail>0 && (
          <div style={{ display:'flex', gap:1, background:'rgba(255,255,255,0.06)', borderRadius:12, overflow:'hidden' }}>
            {[[totalPreds,'Feitos','#F5A623'],[totalAvail-totalPreds,'Pendentes','#00c44f'],[totalAvail,'Disponíveis','#7B8CE8']].map(([v,l,c])=>(
      ...
      {/* Abas de rodada */}
      <div style={{ background:'#0d1b2e', borderBottom:'1px solid rgba(255,255,255,0.08)', padding:'10px 10px', display:'flex', gap:6, overflowX:'auto', position:'sticky', top:58, zIndex:30 }}>
        {ROUNDS.map(r => {
          ...
          return (
            <button key={r.id} onClick={()=>setActiveRound(r.id)} style={{
              ...
              background: active ? '#7B2FBE' : 'rgba(255,255,255,0.06)',
      ...
      <main style={{ padding:'14px 12px' }}>
        {loading ? (
          <div style={{ textAlign:'center', padding:48 }}>
            <Loader2 size={28} style={{ animation:'spin 1s linear infinite', margin:'0 auto', display:'block', color:'#7B2FBE' }}/>
            <style>{`@keyframes spin{from{transform:rotate(0)}to{transform:rotate(360deg)}}`}</style>
          </div>
        ) : (
          <>
            {roundMatches.every(m=>!dbMatches[m.id]?.team1) && (
              <div style={{ background:'rgba(123,47,190,0.1)', border:'1px solid rgba(123,47,190,0.25)', borderRadius:14, padding:'24px', textAlign:'center', marginBottom:14 }}>
                <div style={{ fontSize:36, marginBottom:8 }}>⏳</div>
                <div style={{ color:'#c084fc', fontWeight:800, fontSize:14 }}>Rodada ainda não liberada</div>
                <div style={{ color:'rgba(255,255,255,0.3)', fontSize:12, marginTop:4 }}>O admin irá preencher os times após a fase anterior.</div>
              </div>
            )}
            {roundMatches.map(m => (
