      ...
      {Object.entries(byDate).map(([dateLabel, matches]) => (
        <div key={dateLabel} style={{ marginBottom:18 }}>
          ...
          {matches.map(m => {
            ...
            return (
              <div key={m.id} ref={el => matchRefs && (matchRefs.current[m.id] = el)} style={{ background:'#fff', borderRadius:12, border:'1px solid #E8EDF2', marginBottom:8, overflow:'hidden', boxShadow:'0 1px 4px rgba(0,40,85,0.06)' }}>
                {/* Linha dos times */}
                <div style={{ display:'flex', alignItems:'center' }}>
