...
function MatchRow({ match: m, prediction, onSave }) {
  ...
  useEffect(() => {
    setS1(prediction?.score1??'')
    setS2(prediction?.score2??'')
  ...
  return (
    <div style={{
      ...
      border: m.is_finished ? '1px solid rgba(0,150,57,0.2)' : '1px solid #E2EAF0',
      boxShadow:'0 1px 6px rgba(0,40,85,0.05)',
      opacity: m.is_finished ? 0.85 : 1,
    }}>
      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:10, fontSize:11 }}>
        ...
        <span style={{
          color: m.is_finished ? '#009639' : open ? '#009639' : '#9BABB8',
          fontWeight:700
        }}>
      ...
      {m.is_finished && m.score1 !== null && (
        <div style={{
          ...
          background:'rgba(0,150,57,0.06)', borderRadius:10, padding:'6px',
          border:'1px solid rgba(0,150,57,0.15)'
        }}>
          <span style={{ color:'#6B7A8D', fontSize:11 }}>Resultado final: </span>
          <span style={{ color:'#009639', fontWeight:900, fontSize:13 }}>{m.score1} × {m.score2}</span>
        </div>
      )}

      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:12 }}>
        {b
          ? <span className={b.cls} style={{ borderRadius:20, padding:'3px 12px', fontSize:11, fontWeight:800 }}>{b.label}</span>
          : <span />
        }
        {open && (
        ...
        {!open && !prediction && !m.is_finished && (
          <span style={{ color:'#9BABB8', fontSize:11, fontStyle:'italic' }}>Sem palpite</span>
        )}
      </div>
    </div>
  )
...
export default function Predictions({ participant, onLogout }) {
  ...
  const fetchPreds = useCallback(async () => {
    const { data } = await supabase
      .from('predictions')
      .select('match_id,score1,score2,points')
      .eq('participant_id', participant.id)
    const m = {}
    data?.forEach(p => { m[p.match_id] = p })
    setPreds(m)
  }, [participant.id])
  ...
  useEffect(() => {
    Promise.all([fetchPreds(), fetchResults()]).then(() => setLoading(false))
  ...
  const save = async (id, s1, s2) => {
    if (preds[id]) {
      await supabase.from('predictions').update({ score1:s1, score2:s2, updated_at:new Date().toISOString() })
        .eq('participant_id', participant.id).eq('match_id', id)
    } else {
  ...
  const open = enrichedMatches.filter(m => isMatchOpen(m) && !m.is_finished && !preds[m.id]).length
    ...
    <div style={{ minHeight:'100vh', background:'#F4F6F9', paddingBottom:80 }}>
      ...
      <main style={{ paddingTop:70, padding:'70px 12px 80px' }}>
        <div style={{ marginBottom:16 }}>
          ...
          {open > 0 && (
            <p style={{ color:'#D4890A', fontSize:13, marginTop:4, fontWeight:700 }}>
              ⚠️ {open} jogo{open!==1?'s':''} aguardando seu palpite!
            </p>
          )}
        </div>
        ...
        <div style={{ display:'flex', gap:8, overflowX:'auto', paddingBottom:8, marginBottom:10 }}>
          {FILTERS.map(f => (
            <button key={f.key} onClick={() => setFilter(f.key)} style={{
              ...
              border: filter===f.key ? 'none' : '1.5px solid #E2EAF0',
              fontWeight:800, fontSize:12, cursor:'pointer',
              background: filter===f.key ? '#009639' : '#fff',
              color: filter===f.key ? '#fff' : '#6B7A8D',
        ...
        <div style={{ display:'flex', gap:6, overflowX:'auto', paddingBottom:10, marginBottom:14 }}>
          {GROUPS.map(g => (
            <button key={g} onClick={() => setGrp(g)} style={{
              ...
              border: grp===g ? '2px solid #009639' : '1.5px solid #E2EAF0',
              fontWeight:800, fontSize:11, cursor:'pointer',
              background: grp===g ? '#e8f5ee' : '#fff',
              color: grp===g ? '#009639' : '#6B7A8D',
        ...
        {loading ? (
          <div style={{ textAlign:'center', padding:'48px 0', color:'#9BABB8' }}>
            <Loader2 size={28} className="animate-spin" style={{ margin:'0 auto 12px' }} />
          </div>
        ) : list.length === 0 ? (
          <div style={{ textAlign:'center', padding:'48px 0', color:'#9BABB8' }}>
            <div style={{ fontSize:48 }}>🔍</div>
            <p style={{ marginTop:8 }}>Nenhum jogo neste filtro.</p>
          </div>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {list.map(m => (
              <MatchRow key={m.id} match={m} prediction={preds[m.id]} onSave={save}/>
            ))}
          </div>
        )}
      </main>
